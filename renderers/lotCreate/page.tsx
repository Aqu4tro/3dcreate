...
export function walls({ width, length, tickLot, topSize, size, height, angle_Top }: { width: number; length: number; tickLot: number; topSize: number; size: number; height: number; angle_Top: { f: number; l: number; r: number; b: number } }) { 
  ...
  walls.push({ x: 0, y: (height / 2 - (topSize - tickLot) / 2) + angle_Top.f * 4, z: -length / 2 + sizePin, H: height - (topSize + tickLot) + angle_Top.f * 8 , W: width, L: size , N: "F"});
  walls.push({ x: 0, y: (height / 2 - (topSize - tickLot) / 2) + angle_Top.b * 4, z: length / 2 - sizePin, H: height - (topSize + tickLot)  + (angle_Top.b) * 8 , W: width, L: size, N: "B"});
    ...
    !disable && tickLot && (
      <>
          ...
          <mesh onClick={(event) => switchSelect(event)} position={[position.x, position.y, position.z]} rotation={new THREE.Euler(rotation.x, rotation.y, rotation.z) } >
            ...
            {height && top && (
              <mesh position={[0, height - .05 + (angle_Top.b || angle_Top.f ? angle_Top.f + 0.15 * 2 : 0) ,0]} rotation={new THREE.Euler(angle_Top.b / 4 || -angle_Top.f,0,angle_Top.r)} name="top-surface"> 
            ...
            {height && size && tickLot && walls({ width, height, size, length, tickLot, topSize: .1, angle_Top }).map((e, index) => {
                ...
                switch (e.N) {
                case "R":
                    vertices = new Float32Array(angle_Top.b ? [width, height + angle_Top.f * 8,-length, width, height, length, width, height, -length] : (angle_Top.f ? [1, 2, 3] : []));
                    break;
                  case "L":
                    vertices = new Float32Array(angle_Top.b ? [1, 2, 3] : (angle_Top.f ? [1, 2, 3] : []));
                    break;
                  case "F":
